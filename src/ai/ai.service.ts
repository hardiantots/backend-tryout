...
export class AiService {
  ...
  async generateInsight(examSessionId: string, requesterUserId: string) {
    ...
    return {
      success: true,
      examSessionId,
      prompt: 'Analisis skor 9 sub-tes SNBT ini dan berikan saran belajar taktis menjelang H-1 / H-2 ujian',
      insight: {
  ...
  private async buildFallbackInsight(summary: any): Promise<{
    ...
    const narrative = [
      ...
      weakMaterialLines,
      '',
      '## Saran Belajar H-1 / H-2 Menjelang Tes',
      '- Fokus pada pemantapan 2 sub-tes dengan akurasi terendah; hindari memaksakan diri mempelajari materi baru yang rumit.',
      '- Lakukan repetisi ringan pada soal-soal prioritas yang masih lemah sekadar untuk mengingat pola.',
      '- Jaga fisik dan mental tetap prima; perbanyak istirahat untuk memastikan konsentrasi saat hari ujian tiba.',
      '',
      '## Motivasi Penutup',
  ...
  private buildPrompt(summary: any, strongest: string[], weakest: string[], nameMap: Map<string, string>): string {
    ...
    return [
      ...
      '',
      '## Saran Belajar H-1 / H-2 Menjelang Tes',
      '(3-4 poin bullet berisi strategi taktis dan logis untuk waktu sisa yang sangat singkat: pola pemantapan ringan materi prioritas, saran istirahat mental/fisik, perlunya ketenangan, dll)',
      '',
      '## Motivasi Penutup',
